...
from roadmap.core.domain.issue import Priority, IssueType, Status
...
def measure_all_operations(temp_dir: Path, profiler):
    ...
    try:
        ...
        m1 = core.milestones.create(name="v1.0", description="Release 1")
        m2 = core.milestones.create(name="v1.1", description="Release 1.1")
        m3 = core.milestones.create(name="v2.0", description="Release 2")
        ...
        successful, failed = core.issues.batch_assign_to_milestone(issue_ids[:20], "v1.0", preloaded_issues=all_issues)
...
def main(profile_file: str):
    ...
    try:
        ...
        results_dict["operations_measured"] = len(profiler._operations) if hasattr(profiler, '_operations') else "unknown"
        ...
        click.secho("-" * 70, fg="cyan")
        click.echo(f"Total operations measured: {results_dict.get('total_operations', 'N/A')}")
