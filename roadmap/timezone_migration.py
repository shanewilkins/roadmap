...
class TimezoneDataMigrator:
    ...
    def analyze_data(self) -> dict:
        ...
        try:
            ...
            for milestone in milestones:
                ...
                if (
                    ...
                    and milestone.actual_start_date.tzinfo is None
                ):
                    naive_fields.append("actual_start_date")
                if (
                    milestone.actual_end_date
                    and milestone.actual_end_date.tzinfo is None
                ):
                    naive_fields.append("actual_end_date")
                if (
                    milestone.last_progress_update
                    and milestone.last_progress_update.tzinfo is None
                ):
        ...
        analysis["migration_required"] = (
            analysis["issues_with_naive_dates"] > 0
            or analysis["milestones_with_naive_dates"] > 0
        )
    ...
    def create_backup(self) -> bool:
        ...
        try:
            ...
            if self.core.issues_dir.exists():
                ...
                shutil.copytree(
                    self.core.issues_dir, backup_path / "issues", dirs_exist_ok=True
                )
    ...
    def migrate_issue(self, issue: Issue, assumed_timezone: str = "UTC") -> Issue:
        ...
        if issue.created and issue.created.tzinfo is None:
            migrated_issue.created = migrate_naive_datetime(
                issue.created, assumed_timezone
            )

        if issue.updated and issue.updated.tzinfo is None:
            migrated_issue.updated = migrate_naive_datetime(
                issue.updated, assumed_timezone
            )

        if issue.due_date and issue.due_date.tzinfo is None:
            migrated_issue.due_date = migrate_naive_datetime(
                issue.due_date, assumed_timezone
            )

        if issue.actual_start_date and issue.actual_start_date.tzinfo is None:
            migrated_issue.actual_start_date = migrate_naive_datetime(
                issue.actual_start_date, assumed_timezone
            )

        if issue.actual_end_date and issue.actual_end_date.tzinfo is None:
            migrated_issue.actual_end_date = migrate_naive_datetime(
                issue.actual_end_date, assumed_timezone
            )

        if issue.handoff_date and issue.handoff_date.tzinfo is None:
            migrated_issue.handoff_date = migrate_naive_datetime(
                issue.handoff_date, assumed_timezone
            )

        return migrated_issue

    def migrate_milestone(
        self, milestone: Milestone, assumed_timezone: str = "UTC"
    ) -> Milestone:
        ...
        # Migrate datetime fields
        if milestone.created and milestone.created.tzinfo is None:
            migrated_milestone.created = migrate_naive_datetime(
                milestone.created, assumed_timezone
            )

        if milestone.updated and milestone.updated.tzinfo is None:
            migrated_milestone.updated = migrate_naive_datetime(
                milestone.updated, assumed_timezone
            )

        if milestone.due_date and milestone.due_date.tzinfo is None:
            migrated_milestone.due_date = migrate_naive_datetime(
                milestone.due_date, assumed_timezone
            )

        if milestone.actual_start_date and milestone.actual_start_date.tzinfo is None:
            migrated_milestone.actual_start_date = migrate_naive_datetime(
                milestone.actual_start_date, assumed_timezone
            )

        if milestone.actual_end_date and milestone.actual_end_date.tzinfo is None:
            migrated_milestone.actual_end_date = migrate_naive_datetime(
                milestone.actual_end_date, assumed_timezone
            )
        ...
        if (
            ...
            migrated_milestone.last_progress_update = migrate_naive_datetime(
                milestone.last_progress_update, assumed_timezone
            )

        return migrated_milestone

    def migrate_all_data(
        self, assumed_timezone: str = "UTC", dry_run: bool = False
    ) -> dict:
                        ...
                        try:
                            ...
                            needs_migration = any(
                                [
                                    ...
                                    and issue.actual_start_date.tzinfo is None,
                                    issue.actual_end_date
                                    and issue.actual_end_date.tzinfo is None,
                                    issue.handoff_date
                                    and issue.handoff_date.tzinfo is None,
                                ]
                            )

                            if needs_migration:
                                migrated_issue = self.migrate_issue(
                                    issue, assumed_timezone
                                )
                                ...
                                if not dry_run:
                                    ...
                                    try:
                                        ...
                                        if (
                                            migrated_issue.actual_end_date
                                            != issue.actual_end_date
                                        ):
                                        ...
                                        if (
                                            migrated_issue.handoff_date
                                            != issue.handoff_date
                                        ):
                                            update_fields["handoff_date"] = (
                                                migrated_issue.handoff_date
                                            )
                                        ...
                                        if update_fields:
                                            ...
                                            self.core.update_issue(
                                                migrated_issue.id, **update_fields
                                            )
                                    except Exception:
                                        # If core update fails, try direct file manipulation
                                        issue_file = None
                                        for potential_file in self.core.issues_dir.glob(
                                            f"{migrated_issue.id}-*.md"
                                        ):
                                            issue_file = potential_file
                                            break

                                        if issue_file and issue_file.exists():
                                            self._update_datetime_in_file(
                                                issue_file, issue, migrated_issue
                                            )

                                results["issues_migrated"] += 1
                                self.migration_log.append(
                                    f"✅ Migrated issue: {issue.id} - {issue.title}"
                                )
            ...
            with Progress() as progress:
                if milestones:
                    task = progress.add_task(
                        "Migrating milestones...", total=len(milestones)
                    )
                    ...
                    for milestone in milestones:
                        try:
                            ...
                            needs_migration = any(
                                [
                                    milestone.created
                                    and milestone.created.tzinfo is None,
                                    milestone.updated
                                    and milestone.updated.tzinfo is None,
                                    milestone.due_date
                                    and milestone.due_date.tzinfo is None,
                                    milestone.actual_start_date
                                    and milestone.actual_start_date.tzinfo is None,
                                    milestone.actual_end_date
                                    and milestone.actual_end_date.tzinfo is None,
                            ...
                            if needs_migration:
                                migrated_milestone = self.migrate_milestone(
                                    milestone, assumed_timezone
                                )
                                ...
                                if not dry_run:
                                    ...
                                    try:
                                        ...
                                        if update_fields:
                                            ...
                                            self.core.update_milestone(
                                                migrated_milestone.name, **update_fields
                                            )

                                        # For other datetime fields, update the file directly
                                        milestone_file = (
                                            self.core.milestones_dir
                                            / f"{migrated_milestone.name}.md"
                                        )
                                ...
                                self.migration_log.append(
                                    f"✅ Migrated milestone: {milestone.name}"
                                )
                            ...
                            error_msg = (
                                f"Failed to migrate milestone {milestone.name}: {e}"
                            )
                            results["errors"].append(error_msg)
                            self.migration_log.append(f"❌ {error_msg}")
    ...
    def verify_migration(self) -> dict:
        ...
        verification["migration_successful"] = (
            verification["issues_with_naive_dates"] == 0
            and verification["milestones_with_naive_dates"] == 0
        )
    ...
    def print_analysis_report(self, analysis: dict) -> None:
        ...
        table = Table(
            title="Timezone Migration Analysis",
            show_header=True,
        ...
        table.add_row(
            "Issues with Naive Dates",
            str(analysis["issues_with_naive_dates"]),
        ...
        table.add_row(
            "Milestones with Naive Dates",
            str(analysis["milestones_with_naive_dates"]),
            f"{(analysis['milestones_with_naive_dates']/max(1,analysis['total_milestones'])*100):.1f}%",
        )

        table.add_row(
            "Unique Date Fields",
            str(len(analysis["unique_naive_fields"])),
            ", ".join(analysis["unique_naive_fields"][:5]),
        )

        migration_status = (
            "✅ Required" if analysis["migration_required"] else "✅ Not Required"
        )
    ...
    def print_migration_report(self, results: dict) -> None:
        ...
        table.add_row(
            "Milestones Migrated", str(results["milestones_migrated"]), "✅ Success"
        )
        table.add_row(
            "Errors",
    ...
    def save_migration_log(self) -> Path:
        ...
        log_file = (
            self.backup_dir / f"migration_log_{now_utc().strftime('%Y%m%d_%H%M%S')}.txt"
        )
        ensure_directory_exists(log_file.parent)

        with open(log_file, "w") as f:
            f.write("Timezone Migration Log\n")
    ...
    def _update_datetime_in_file(
        self, file_path: Path, original_obj, migrated_obj
    ) -> None:
                ...
                if len(parts) >= 3:
                    ...
                    for field in datetime_fields:
                        ...
                        if (
                            ...
                            and migrated_val is not None
                            and original_val != migrated_val
                        ):
