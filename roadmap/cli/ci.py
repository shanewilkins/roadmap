...
def track_commit(commit_sha: str, message: Optional[str], issue_id: Optional[str]):
...
@click.option('--state', type=click.Choice(['open', 'closed', 'merged']), required=True, help='Pull request state')
        ...
        if state == 'open':
            ...
            if branch_results:
                for issue_id, branch_actions in branch_results.items():
                    actions_taken.extend([f"Associated PR with issue {issue_id}"] + branch_actions)
            else:
    ...
    try:
        ...
        if format == 'table':
            ...
            if roadmap_core.is_initialized():
                table.add_row("Roadmap Setup", "✅ Ready", "Roadmap initialized and configured")
            else:
                table.add_row("Roadmap Setup", "❌ Not Ready", "Run 'roadmap init' first")
            
            # Check CI configuration  
            table.add_row("Auto Tracking", "✅ Enabled" if config.auto_start_on_branch else "⚠️ Disabled", 
                         "Automatic issue tracking on branch creation")
            table.add_row("Progress Tracking", "✅ Enabled" if config.auto_progress_on_pr else "⚠️ Disabled",
                         "Automatic progress updates from commits") 
            table.add_row("Auto Completion", "✅ Enabled" if config.auto_close_on_merge else "⚠️ Disabled",
            ...
            if os.path.exists(workflows_dir):
                ...
                if workflow_files:
                    table.add_row("GitHub Workflows", "✅ Found", f"{len(workflow_files)} workflow file(s)")
                else:
                    table.add_row("GitHub Workflows", "⚠️ None", "No workflow files found")
            else:
                table.add_row("GitHub Workflows", "❌ Missing", "No .github/workflows directory")
...
@ci.command()
@click.argument('workflow_name', type=click.Choice(['starter', 'integration', 'ci-cd', 'lifecycle']))
@click.option('--output-dir', default='.github/workflows', help='Directory to create workflow files')
...
def setup_workflows(workflow_name: str, output_dir: str, force: bool):
    ...
    try:
        ...
        if source_path.exists():
            ...
            # Show next steps
            rprint(f"\n📋 Next steps:")
            rprint(f"  1. Review and customize the workflow file")
            rprint(f"  2. Commit and push to trigger the workflow")
            rprint(f"  3. Check the Actions tab in your GitHub repository")
            
            if workflow_name == 'starter':
                rprint(f"\n💡 For more advanced features, try:")
                rprint(f"  roadmap ci setup-workflows integration")
...
@click.option('--create-issues/--no-create-issues', default=False, help='Create missing issues from commits')
@click.option('--link-commits/--no-link-commits', default=True, help='Link commits to existing issues')
def scan_full_repository(max_commits: int, max_branches: int, since: Optional[str], 
                        until: Optional[str], parallel: bool, workers: int, 
                        export: Optional[str], create_issues: bool, link_commits: bool):
    ...
    try:
        ...
        rprint(f"🔍 Starting comprehensive repository scan...")
        ...
        if scan_result.branches:
            ...
            branch_types = Counter(b.branch_type or 'other' for b in scan_result.branches)
        ...
        if create_issues or link_commits:
            ...
            if create_issues:
                created_count = sum(1 for r in migration_result.results 
            ...
            if link_commits:
                linked_count = sum(r.get('new_commits', 0) for r in migration_result.results 
                                 if r.get('action') == 'linked_commits')
                migration_table.add_row(
                    "Commits Linked",
                    str(linked_count), 
                    f"To existing issues"
                )
            
            if migration_result.warnings:
                migration_table.add_row(
                    "Warnings",
                    str(len(migration_result.warnings)),
                    "See details above"
                )
...
@click.option('--max-commits', default=2000, help='Maximum commits to analyze for migration')
@click.option('--create-issues/--no-create-issues', default=False, 
              help='Create missing issues discovered in commit history')
@click.option('--link-commits/--no-link-commits', default=True,
              help='Automatically link commits to existing issues')
@click.option('--dry-run/--execute', default=False, help='Preview migration without making changes')
@click.option('--export-report', help='Export migration report to JSON file')
def migrate_project(max_commits: int, create_issues: bool, link_commits: bool, 
                   dry_run: bool, export_report: Optional[str]):
    ...
    try:
        ...
        rprint(f"🚀 Starting project migration...")
        ...
        if scan_result.issue_associations:
            ...
            for issue_id, commit_shas in sorted_candidates:
                ...
                sample_commit = next(
                    (c for c in scan_result.commits if c.sha in commit_shas),
                    None
                )
                sample_msg = sample_commit.message[:50] + "..." if sample_commit else "N/A"
        ...
        if not dry_run:
            ...
            with Progress(
                ...
                progress.update(migrate_task, completed=True, description="Migration completed")
            ...
            if create_issues:
                created_issues = [r for r in migration_result.results 
                                if r.get('action') == 'created_issue']
                result_table.add_row(
                    "Issues Created",
                    str(len(created_issues)),
                    f"From discovered commit references"
                )
            
            if link_commits:
                linked_operations = [r for r in migration_result.results 
                                   if r.get('action') == 'linked_commits']
            ...
            result_table.add_row(
                "Duration",
                f"{migration_result.duration:.2f}s",
            ...
            rprint(f"   To perform migration, run without --dry-run flag")
        ...
        if export_report:
            report_data = {
                "migration_summary": {
                    ...
                    "would_link_commits": scan_result.commits_with_issues if link_commits else 0
                },
                "issue_candidates": [
                    {
                        "issue_id": issue_id,
                        "commit_count": len(commit_shas),
                        "exists": (roadmap_core.issues_dir / f"{issue_id}.yaml").exists()
                    }
        ...
        if not dry_run:
            ...
            rprint(f"   Your repository is now integrated with roadmap tracking")
        else:
            rprint(f"\n📋 [bold blue]Migration preview completed[/bold blue]")
            rprint(f"   Review the analysis and run without --dry-run when ready")
...
@click.option('--pattern-type', type=click.Choice(['commit', 'branch', 'both']), 
              default='both', help='Type of patterns to analyze')
@click.option('--export', help='Export analysis to JSON file')
def analyze_patterns(commits: int, pattern_type: str, export: Optional[str]):
    ...
    try:
        ...
        rprint(f"🔍 Analyzing repository patterns...")
        ...
        if pattern_type in ['commit', 'both']:
            ...
            from collections import Counter
            commit_types = Counter(c.commit_type for c in scan_result.commits if c.commit_type)
            ...
            if commits_with_issues:
                ...
                rprint(f"   Commits with issue refs: {len(commits_with_issues)}/{len(scan_result.commits)} ({len(commits_with_issues)/len(scan_result.commits)*100:.1f}%)")
                
                # Common patterns in messages
                issue_messages = [c.message for c in commits_with_issues]
                rprint(f"   Sample messages with issues:")
        ...
        if pattern_type in ['branch', 'both']:
            ...
            if branches_with_issues:
                ...
                rprint(f"   Branches with issue IDs: {len(branches_with_issues)}/{len(scan_result.branches)} ({len(branches_with_issues)/len(scan_result.branches)*100:.1f}%)")
                
                # Sample branch names
                rprint(f"   Sample branches with issues:")
        ...
        commits_with_progress = len([c for c in scan_result.commits if c.progress_markers])
        ...
        if export:
            analysis_data = {
                ...
                "commit_patterns": {
                    "type_distribution": dict(Counter(c.commit_type for c in scan_result.commits if c.commit_type)),
                    "issue_reference_rate": commits_with_refs / len(scan_result.commits) if scan_result.commits else 0,
                    "sample_messages": [c.message for c in scan_result.commits[:10]]
                },
                "branch_patterns": {
                    "type_distribution": dict(Counter(b.branch_type or 'other' for b in scan_result.branches)),
                    "issue_association_rate": branches_with_refs / len(scan_result.branches) if scan_result.branches else 0,
