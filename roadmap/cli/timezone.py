...
from ..timezone_utils import get_timezone_manager, TimezoneManager
...
def timezone_status(ctx):
    ...
    console.print(f"🌍 Timezone Manager Status", style="bold blue")
    ...
    if analysis["migration_required"]:
        ...
        console.print("Run 'roadmap timezone migrate' to convert naive datetimes to timezone-aware format")
    else:
        console.print(f"\n✅ All datetime data is timezone-aware", style="bold green")


@timezone_group.command("migrate")
@click.option("--timezone", default="UTC", help="Timezone to assume for existing naive datetimes")
...
def migrate_timezone_data(ctx, timezone: str, dry_run: bool, force: bool):
    ...
    if not analysis["migration_required"]:
        console.print("✅ No migration required - all data is already timezone-aware", style="green")
        return
    
    console.print(f"🔍 Migration Analysis:", style="bold blue")
    migrator.print_analysis_report(analysis)
    ...
    console.print(f"   • Milestones to migrate: {analysis['milestones_with_naive_dates']}")
    ...
    migrator.print_migration_report(results["results"] if "results" in results else results)
    ...
    if not dry_run:
        ...
        if verification["migration_successful"]:
            ...
            console.print(f"   • {verification['milestones_checked']} milestones verified")
        else:
            console.print("⚠️  Migration verification failed:", style="bold yellow")
            console.print(f"   • {verification['issues_with_naive_dates']} issues still have naive dates")
            console.print(f"   • {verification['milestones_with_naive_dates']} milestones still have naive dates")
...
@click.pass_context
def timezone_config(ctx, timezone: Optional[str], show: bool):
    ...
    if show or (not timezone):
        console.print(f"🌍 Current Timezone Configuration:", style="bold blue")
        console.print(f"   User Timezone: {tz_manager.user_timezone}")
    ...
    try:
        ...
        console.print(f"ℹ️  Note: Timezone preference is set for this session only.", style="blue")
        console.print("Future versions will support persistent timezone configuration.")
        
    except ValueError as e:
        console.print(f"❌ Invalid timezone '{timezone}': {e}", style="bold red")
        console.print("Use 'roadmap timezone config --show' to see available timezones.")
...
@click.pass_context
def test_timezone_parsing(ctx, date_string: str, input_timezone: Optional[str], display_timezone: Optional[str]):
    """Test timezone parsing and formatting with different inputs."""
    tz_manager = get_timezone_manager(display_timezone)
    
    console.print(f"🧪 Testing Timezone Parsing:", style="bold blue")
    console.print(f"   Input: '{date_string}'")
    console.print(f"   Input Timezone: {input_timezone or tz_manager.user_timezone}")
    console.print(f"   Display Timezone: {display_timezone or tz_manager.user_timezone}")
    ...
    try:
        ...
        console.print(f"Examples of valid formats:", style="blue")
        console.print(f"   • 2025-12-31")
        console.print(f"   • 2025-12-31 23:59:59")
        console.print(f"   • 2025-12-31T23:59:59")
        console.print(f"   • 2025-12-31T23:59:59Z")
        console.print(f"   • 2025-12-31T23:59:59+05:00")