...
def milestone_status(ctx: click.Context, milestone_name: str, show_issues: bool):
    ...
    try:
        ...
        progress_value = (
            ...
            or milestone.get_completion_percentage(all_issues)
        )
        ...
        if milestone.actual_start_date:
            console.print(
                f"Started: {milestone.actual_start_date.strftime('%Y-%m-%d')}"
            )

        if milestone.actual_end_date:
            console.print(
                f"Completed: {milestone.actual_end_date.strftime('%Y-%m-%d')}"
            )
        ...
        if show_issues:
            ...
            if milestone_issues:
                ...
                for issue in sorted(milestone_issues, key=lambda x: x.status.value):
                    ...
                    progress_str = (
                        f"{issue.progress_percentage:.0f}%"
...
@click.argument("project_id")
@click.option(
    "--show-milestones", is_flag=True, help="Show detailed milestone breakdown"
)
@click.option("--show-timeline", is_flag=True, help="Show timeline information")
@click.pass_context
def project_status(
    ctx: click.Context, project_id: str, show_milestones: bool, show_timeline: bool
):
    ...
    try:
        ...
        if show_timeline:
            ...
            if project.target_end_date:
                console.print(
                    f"Target End: {project.target_end_date.strftime('%Y-%m-%d')}"
                )
            if project.projected_end_date:
                console.print(
                    f"Projected End: {project.projected_end_date.strftime('%Y-%m-%d')}"
            ...
            if project.actual_end_date:
                console.print(
                    f"Actual End: {project.actual_end_date.strftime('%Y-%m-%d')}"
                )
            if project.schedule_variance:
                variance_str = f"{abs(project.schedule_variance)} days {'ahead' if project.schedule_variance < 0 else 'behind'}"
                console.print(f"Schedule Variance: {variance_str}")
            if project.completion_velocity:
                console.print(
                    f"Velocity: {project.completion_velocity:.2f} milestones/week"
                )
