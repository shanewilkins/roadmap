...
from .. import core
from ..version import VersionManager, SemanticVersion
...
def version_check(ctx):
    ...
    if result['consistent']:
        ...
        console.print(f"✅ Version consistency check passed", style="bold green")
        console.print(f"📦 Current version: {current_version}", style="blue")
    else:
        console.print(f"❌ Version consistency issues found:", style="bold red")
        for issue in result['issues']:
            console.print(f"   • {issue}", style="red")
        
        console.print(f"\n📋 Current versions:", style="bold")
        console.print(f"   pyproject.toml: {result['pyproject_version'] or 'Not found'}")
    ...
    if git_status['is_git_repo']:
        ...
        if git_status['is_clean']:
            console.print(f"   Status: Clean working directory", style="green")
        else:
            console.print(f"   Status: {len(git_status['uncommitted_files'])} uncommitted files", style="yellow")
...
def prepare_release(ctx, bump_type: str, dry_run: bool):
    ...
    if git_status['is_git_repo'] and not git_status['is_clean']:
        ...
        if len(git_status['uncommitted_files']) > 5:
            console.print(f"   • ... and {len(git_status['uncommitted_files']) - 5} more", style="yellow")
    ...
    if dry_run:
        ...
        console.print(f"   • Generate changelog entry with {len(completed_issues)} completed issues")
        ...
        if version_manager.update_version(new_version):
            ...
            console.print(f"❌ Failed to update version", style="red")
            return
        ...
        if version_manager.update_changelog(new_version, changelog_entry):
            console.print(f"✅ Updated changelog", style="green")
        else:
            console.print(f"❌ Failed to update changelog", style="red")
            return
        
        console.print(f"\n🎉 Release {new_version} prepared successfully!", style="bold green")
        console.print(f"📝 Updated {len(completed_issues)} issues in changelog", style="blue")
        
        # Show next steps
        console.print(f"\n📋 Next steps:", style="bold")
        console.print(f"   1. Review the changes: git diff")
        console.print(f"   2. Commit the release: git add . && git commit -m 'Release {new_version}'")
        console.print(f"   3. Tag the release: git tag v{new_version}")
        console.print(f"   4. Build the package: poetry build")
        console.print(f"   5. Publish to PyPI: poetry publish")


@release_group.command("build")
@click.option("--check", is_flag=True, help="Check if the package can be built without building")
@click.pass_context
def build_package(ctx, check: bool):
    ...
    try:
        ...
        console.print("❌ Poetry not found. Make sure Poetry is installed and in PATH.", style="bold red")
...
def release_status(ctx):
    ...
    if version_result['consistent']:
        table.add_row("Version Consistency", "✅ Pass", f"v{version_result['pyproject_version']}")
    else:
    ...
    if git_status['is_git_repo']:
        if git_status['is_clean']:
            table.add_row("Git Status", "✅ Clean", f"Branch: {git_status['current_branch']}")
        else:
            table.add_row("Git Status", "⚠️  Dirty", f"{len(git_status['uncommitted_files'])} uncommitted files")
    else:
    ...
    if in_progress_issues == 0:
        table.add_row("Open Issues", "✅ None", f"{completed_issues}/{total_issues} completed")
    else:
        table.add_row("Open Issues", "⚠️  Active", f"{in_progress_issues} in progress, {completed_issues}/{total_issues} completed")
    ...
    table.add_row("Milestones", "ℹ️  Info", f"{completed_milestones} completed, {active_milestones} active")
    ...
    console.print()
    if (version_result['consistent'] and 
        git_status.get('is_clean', True) and 
        in_progress_issues == 0):
        console.print("🚀 Ready for release!", style="bold green")
        console.print("Run 'roadmap release prepare [patch|minor|major]' to start", style="blue")
    else:
