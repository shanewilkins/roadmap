...
from .models import Issue, Milestone, Project, Status, MilestoneStatus, ProjectStatus, RiskLevel
...
class ProgressCalculationEngine:
    ...
    def update_milestone_progress(self, milestone: Milestone, all_issues: List[Issue]) -> bool:
        ...
        if progress_changed or status_changed:
            old_progress_str = f"{old_progress:.1f}" if old_progress is not None else "None"
            new_progress_str = f"{milestone.calculated_progress:.1f}" if milestone.calculated_progress is not None else "None"
            logger.info(f"Updated milestone '{milestone.name}': progress {old_progress_str}% -> {new_progress_str}%, status {old_status} -> {milestone.status}")
            return True
        
        return False
    
    def update_project_progress(self, project: Project, all_milestones: List[Milestone], all_issues: List[Issue]) -> bool:
        ...
        if progress_changed or status_changed:
            old_progress_str = f"{old_progress:.1f}" if old_progress is not None else "None"
            new_progress_str = f"{project.calculated_progress:.1f}" if project.calculated_progress is not None else "None"
            logger.info(f"Updated project '{project.name}': progress {old_progress_str}% -> {new_progress_str}%, status {old_status} -> {project.status}")
            return True
        
        return False
    
    def update_issue_dependencies(self, updated_issue: Issue, all_issues: List[Issue], all_milestones: List[Milestone], all_projects: List[Project]) -> Tuple[List[Milestone], List[Project]]:
    ...
    def recalculate_all_progress(self, all_issues: List[Issue], all_milestones: List[Milestone], all_projects: List[Project]) -> Dict[str, int]:
        ...
        logger.info(f"Recalculated progress: {updated_milestones} milestones, {updated_projects} projects updated")
    ...
    def _update_project_timeline(self, project: Project, all_milestones: List[Milestone], all_issues: List[Issue]) -> None:
        ...
        projected_date = self._calculate_projected_completion(project, project_milestones, all_issues)
        if projected_date:
    ...
    def _calculate_completion_velocity(self, milestones: List[Milestone], all_issues: List[Issue], window_weeks: int = 4) -> Optional[float]:
        ...
        for milestone in milestones:
            if (milestone.actual_end_date and 
                milestone.actual_end_date >= cutoff_date and
    ...
    def _calculate_projected_completion(self, project: Project, milestones: List[Milestone], all_issues: List[Issue]) -> Optional[datetime]:
        ...
        incomplete_milestones = [m for m in milestones if m.status != MilestoneStatus.CLOSED]
...
class ProgressEventSystem:
    ...
    def on_issue_updated(self, updated_issue: Issue, changes: Dict[str, any], all_issues: List[Issue], all_milestones: List[Milestone], all_projects: List[Project]) -> Dict[str, any]:
        ...
        progress_affecting_changes = ['status', 'progress_percentage', 'milestone', 'estimated_hours']
