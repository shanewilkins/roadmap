...
class FileOperationError(Exception):
    ...
    def __init__(self, message: str, path: Optional[Path] = None, operation: Optional[str] = None):
        ...
        file_logger.error(f"FileOperationError: {message} (path: {path}, operation: {operation})")
...
@contextmanager
def SecureFileManager(file_path: Union[str, Path], mode: str = "w", **kwargs):
...
def cleanup_temp_files(directory: Union[str, Path], pattern: str = "*.tmp") -> int:
    ...
    try:
        ...
        raise FileOperationError(f"Failed to cleanup temp files: {e}", directory, "cleanup")