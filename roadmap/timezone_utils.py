...
class TimezoneManager:
    ...
    def parse_user_input(self, date_str: str, input_timezone: Optional[str] = None) -> datetime:
        ...
        try:
            ...
            if 'T' in date_str:
                ...
                if date_str.endswith('Z'):
                    ...
                    return naive_dt.replace(tzinfo=ZoneInfo(input_tz)).astimezone(timezone.utc)
            else:
    ...
    def make_aware(self, dt: datetime, tz: Optional[str] = None) -> datetime:
...
_global_timezone_manager: Optional[TimezoneManager] = None


def get_timezone_manager(user_timezone: Optional[str] = None) -> TimezoneManager:
...
def now_local(user_timezone: Optional[str] = None) -> datetime:
    """Convenience function to get current datetime in user's timezone."""
    return get_timezone_manager(user_timezone).now_local()


def parse_datetime(date_str: str, input_timezone: Optional[str] = None) -> datetime:
...
def format_relative_time(dt: datetime, user_timezone: Optional[str] = None) -> str:
