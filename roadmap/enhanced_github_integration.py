...
from .error_handling import ErrorHandler, GitOperationError, NetworkError
...
class EnhancedGitHubIntegration:
    ...
    def __init__(
        self, roadmap_core: RoadmapCore, github_client: GitHubClient | None = None
    ):
        ...
        self.sync = (
            None  # Simplified for now - could integrate with existing sync later
        )
            ...
            if "github_owner" in repo_info and "github_repo" in repo_info:
                try:
                    self.github_client = GitHubClient(
                        owner=repo_info["github_owner"], repo=repo_info["github_repo"]
                    )
    ...
    def create_github_issue_from_roadmap(
        self, roadmap_issue: Issue
    ) -> dict[str, Any] | None:
        ...
        try:
            ...
            if roadmap_issue.milestone:
                milestone_number = self._get_github_milestone_number(
                    roadmap_issue.milestone
                )
                if milestone_number:
            ...
            self.core.update_issue(
                roadmap_issue.id, github_issue=github_issue["number"]
            )
    ...
    def sync_issue_with_github(
        self, issue_id: str, direction: str = "bidirectional"
    ) -> bool:
    ...
    def handle_pull_request_event(
        self, pr_data: dict[str, Any], action: str
    ) -> list[str]:
        ...
        try:
            ...
            issue_ids = self._extract_issue_references_from_text(
                pr_title + " " + pr_body
            )
            ...
            error_handler.handle_error(
                GitOperationError(
                    ...
                    context={
                        "action": action,
                        "operation": "handle_pull_request_event",
                    },
                    cause=e,
                ),
                show_traceback=False,
        ...
        try:
            ...
            if prs:
                ...
                if pr.get("head"):
                    ...
                    result["deployable"] = result["checks_passing"] and pr.get(
                        "mergeable", False
                    )
    ...
    def enforce_branch_policy(self, branch_name: str) -> dict[str, Any]:
        ...
        try:
            ...
            if not re.match(
                r"^(feature|bugfix|hotfix|docs|test)/[a-f0-9]{8}-[\w-]+$", branch_name
            ):
            ...
            if self.is_github_enabled():
                ...
                if conflict_check.get("conflicts"):
                    result["errors"].append(
                        "Branch has merge conflicts with main branch"
                    )
                    result["valid"] = False
    ...
    def _get_github_milestone_number(self, milestone_name: str) -> int | None:
    ...
    def _update_github_from_roadmap(
        self, roadmap_issue: Issue, github_issue: dict[str, Any]
    ) -> bool:
        ...
        try:
            ...
            self.github_client.update_issue(
                issue_number=roadmap_issue.github_issue, **update_data
            )
            return True
    ...
    def _update_roadmap_from_github(
        self, roadmap_issue: Issue, github_issue: dict[str, Any]
    ) -> bool:
    ...
    def _update_issue_from_pr_event(
        self, issue_id: str, pr_data: dict[str, Any], action: str
    ) -> bool:
    ...
    def _find_prs_for_issue(self, issue: Issue) -> list[dict[str, Any]]:
        ...
        try:
            ...
            query = (
                f"repo:{self.github_client.owner}/{self.github_client.repo} {issue.id}"
            )

            response = self.github_client._make_request(
                "GET",
                "/search/issues",
                params={"q": query + " type:pr", "sort": "updated", "order": "desc"},
            )
