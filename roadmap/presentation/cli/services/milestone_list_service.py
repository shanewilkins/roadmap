...
class MilestoneProgressService:
    ...
    def get_milestone_progress(
        core: RoadmapCore, milestone_name: str
    ) -> dict:
        ...
        try:
            ...
            return {
                ...
                "percentage": (
                    (progress.get("completed", 0) / progress.get("total", 1))
                    * 100
    ...
    @staticmethod
    def get_all_milestones_progress(
        core: RoadmapCore, milestones: list
    ) -> dict:
        ...
        progress_data = {}
        for ms in milestones:
            progress_data[ms.name] = (
                MilestoneProgressService.get_milestone_progress(core, ms.name)
            )
...
class MilestoneTimeEstimateService:
    ...
    @staticmethod
    def get_milestone_time_estimate(
        milestone, all_issues: list
    ) -> str:
...
class MilestoneListService:
    ...
    def get_milestones_list_data(
        self, overdue_only: bool = False
    ) -> dict:
    ...
    def get_milestone_due_date_status(
        self, milestone
        ...
        try:
            ...
            if ms_due_date < now and milestone.status.value == "open":
                ...
                (ms_due_date - now).days <= 7
                and milestone.status.value == "open"
            ):
