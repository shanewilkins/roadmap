...
class GitHubWebhookServer:
    ...
    def __init__(self, roadmap_core: RoadmapCore, secret: str | None = None):
    ...
    async def logging_middleware(self, request: Request, handler) -> Response:
        ...
        try:
            ...
            self.logger.info(
                f"{request.method} {request.path} " f"{response.status} {duration:.3f}s"
            )
            ...
            duration = (datetime.now() - start_time).total_seconds()
            self.logger.error(
                f"{request.method} {request.path} " f"ERROR {duration:.3f}s: {e}"
            )
            raise
    ...
    async def handle_github_webhook(self, request: Request) -> Response:
        ...
        try:
            ...
            # Log event
            self.logger.info(f"Received {event_type} event (delivery: {delivery_id})")
    ...
    async def _handle_event(
        ...
        try:
            if event_type == "push":
                result["updated_issues"] = self.github_integration.handle_push_event(
                    data
                )
                result["handled"] = True
                ...
                result["updated_issues"] = (
                    self.github_integration.handle_pull_request_event(pr_data, action)
                )
                result["handled"] = True
...
class WebhookCLI:
    ...
    @staticmethod
    def start_server(
        host: str = "0.0.0.0", port: int = 8080, secret: str | None = None
    ):
