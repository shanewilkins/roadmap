...
from roadmap.models import Issue, Status, Priority, IssueType
...
class TestCICommandsIntegration:
    ...
    def setup_method(self):
        ...
        subprocess.run(['git', 'config', 'user.name', 'CLI Test'], cwd=self.repo_path, check=True)
        subprocess.run(['git', 'config', 'user.email', 'cli@test.com'], cwd=self.repo_path, check=True)
    ...
    def create_test_git_history(self):
        ...
        commits = [
            ...
            ('cli12345: Add CLI feature implementation', 'cli_feature.py', '# CLI Feature\\n'),
            ('cli12345: Update implementation [progress:50%]', 'cli_feature.py', '# CLI Feature\\n# Updated\\n'),
            ('test6789: Fix CLI bug', 'cli_feature.py', '# CLI Feature\\n# Bug fixed\\n'),
            ('cli12345: Complete feature [closes roadmap:cli12345]', 'cli_feature.py', '# CLI Feature\\n# Complete\\n'),
        ...
        subprocess.run(['git', 'checkout', '-b', 'feature/cli12345-test-cli'], cwd=self.repo_path, check=True)
        (self.repo_path / 'feature_branch.py').write_text('# Feature branch\\n')
        subprocess.run(['git', 'add', 'feature_branch.py'], cwd=self.repo_path, check=True)
        subprocess.run(['git', 'commit', '-m', 'cli12345: Add feature branch code'], cwd=self.repo_path, check=True)
    ...
    def test_ci_config_commands(self):
        ...
        result = self.runner.invoke(ci, ['config', 'set', 'auto_start_on_branch', 'false'])
        assert result.exit_code == 0
        ...
        result = self.runner.invoke(ci, ['config', 'set', 'auto_start_on_branch', 'true'])
        assert result.exit_code == 0
    ...
    def test_track_branch_command(self):
        ...
        assert ("cli12345" in result.output or "No issue IDs found" in result.output or
                "Use pattern like" in result.output)
    ...
    def test_track_commit_command(self):
        ...
        git_result = subprocess.run(['git', 'log', '--format=%H', '-1'], 
        ...
        result = self.runner.invoke(ci, ['track-commit', commit_sha, '--issue-id', 'cli12345'])
        assert result.exit_code == 0
    ...
    def test_scan_repository_command(self):
        ...
        assert ("Repository Scan Results" in result.output or 
                "No issue associations found" in result.output)
    ...
    def test_scan_branches_command(self):
        ...
        assert result.exit_code == 0
        assert ("Scanning git branches" in result.output or
                "Scanning all branches for issue associations" in result.output)
        
        # Should either find associations or report none found
        assert ("feature/cli12345-test-cli" in result.output or 
                "cli12345" in result.output or
                "No issue associations found" in result.output)
    
    def test_scan_full_command(self):
        """Test comprehensive repository scanning."""
        result = self.runner.invoke(ci, ['scan-full', '--max-commits', '15', '--max-branches', '5'])
        ...
        assert ("Top Issues by Commit Activity" in result.output or
                "Issues with Commits │     0" in result.output or
                "No issue associations found" in result.output)
    ...
    def test_migrate_project_dry_run(self):
        ...
        result = self.runner.invoke(ci, ['migrate-project', '--dry-run', '--max-commits', '10'])
        ...
        assert ("Issue Candidates for Migration" in result.output or
                "Top Issue Candidates for Migration" in result.output or
                "Issue IDs Found  │     0" in result.output or
                "No issue IDs found" in result.output)
    
    def test_migrate_project_execution(self):
        """Test actual project migration."""
        result = self.runner.invoke(ci, ['migrate-project', '--link-commits', '--max-commits', '8'])
        ...
        # Should show results
        assert ("Commits Linked" in result.output or 
                "Issues Created" in result.output)
    ...
    def test_analyze_patterns_command(self):
        ...
        result = self.runner.invoke(ci, ['analyze-patterns', '--commits', '8', 
                                        '--export', str(export_file)])
    ...
    def test_setup_workflows_command(self):
        ...
        assert ("Setting up GitHub Actions workflow" in result.output or
                "Created workflow" in result.output or
                "Workflow file already exists" in result.output)
    ...
    def test_track_pr_command(self):
        ...
        result = self.runner.invoke(ci, ['track-pr', '--number', '123', 
                                        '--title', 'Test PR', '--branch', 'feature/test',
                                        '--state', 'open'])
        ...
        result = self.runner.invoke(ci, ['track-pr', '--number', '124', 
                                        '--title', 'cli12345: Fix issue', 
                                        '--branch', 'feature/cli12345-fix',
                                        '--state', 'open'])
        assert result.exit_code == 0
    ...
    def test_cli_help_commands(self):
        ...
        commands = ['status', 'config', 'track-branch', 'track-commit', 
                   'scan-repository', 'scan-branches', 'scan-full',
                   'migrate-project', 'analyze-patterns', 'hooks',
                   'github-status', 'setup-workflows', 'track-pr', 'sync-github']
