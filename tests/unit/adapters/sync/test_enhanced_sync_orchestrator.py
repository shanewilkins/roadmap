"""Tests for EnhancedSyncOrchestrator with git-based baselines."""
...
from roadmap.adapters.sync.enhanced_sync_orchestrator import EnhancedSyncOrchestrator
...
def enhanced_orchestrator(mock_core, mock_backend):
    """Create EnhancedSyncOrchestrator."""
    with patch(
        "roadmap.adapters.sync.enhanced_sync_orchestrator.BaselineStateRetriever"
    ):
        orchestrator = EnhancedSyncOrchestrator(
            core=mock_core,
...
class TestEnhancedSyncOrchestratorInitialization:
    """Test EnhancedSyncOrchestrator initialization."""
...
class TestBuildBaselineStateFromGit:
    ...
    def test_reconstructs_baseline_from_git(self, enhanced_orchestrator, tmp_path):
        ...
        # Mock core.issues.list()
        mock_issue = Mock(spec=Issue)
        mock_issue.id = issue_id
        enhanced_orchestrator.core.issues.list = Mock(return_value=[mock_issue])
