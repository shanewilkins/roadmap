...
class TestStatusDistributionChart:
    ...
    def test_generate_status_distribution_invalid_chart_type(
        self, chart_generator, sample_issues
    ):
        """Test status distribution with invalid chart type."""
        # Just test that it doesn't crash - implementation may handle gracefully
        try:
            result = chart_generator.generate_status_distribution_chart(
                sample_issues, chart_type="invalid"
            )
...
class TestBurndownChart:
    ...
    def test_generate_burndown_chart_success(
        ...
        result = chart_generator.generate_burndown_chart(
            issues, milestone_name=milestone.name
        )
...
class TestVelocityChart:
    ...
    def test_generate_velocity_chart_no_completed_issues(self, chart_generator):
        ...
        with pytest.raises(
            (VisualizationError, KeyError)
        ):  # May raise different errors
...
class TestMilestoneProgressChart:
    ...
    def test_generate_milestone_progress_chart_success(
        ...
        result = chart_generator.generate_milestone_progress_chart(
            milestones_with_progress, issues
        )
...
class TestDashboardGenerator:
    ...
    def test_generate_stakeholder_dashboard_success(
        ...
        mock_chart_gen = Mock()
        mock_chart_gen.generate_status_distribution_chart.return_value = Path(
            "status.png"
        )
        mock_chart_gen.generate_milestone_progress_chart.return_value = Path(
            "milestones.png"
        )
...
class TestVisualizationIntegration:
    ...
    def test_data_consistency_across_charts(
        self, chart_generator, comprehensive_dataset
    ):
