...
@pytest.mark.parametrize("target,message,options,expected_exit", [
    ...
    pytest.param("issue-123", "x" * 10000, [], [0, 1], id="long_message"),  # very long message
])
...
@pytest.mark.parametrize("target,options,expected_exit", [
    ("issue-123", [], [0, 1, 2]),
...
@pytest.mark.parametrize("comment_id,text,expected_exit", [
...
@pytest.mark.parametrize("command,args,expected_exit", [
    (setup_git, [], [0, 1, 2]),
    (install_hooks, [], [0, 1, 2]),
    (hooks_status, [], [0, 1, 2]),
])
