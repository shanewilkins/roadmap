...
class TestDuplicateDetectionPipeline:
    ...
    def test_resolution_action_counts_are_accurate(self):
        ...
        resolver = DuplicateResolver(
            issue_service=mock_service, auto_resolve_threshold=0.95
        )
        ...
        # Should have 1 action (link action during analysis phase)
        assert len(actions) == 1
        assert actions[0].action_type == "link"
        # During analysis phase, no service methods should be called
        assert not mock_service.merge_issues.called
