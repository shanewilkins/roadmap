...
class TestGitHubWebhookServer:
    ...
    def webhook_server(self, mock_core):
        ...
        with patch(
            "roadmap.webhook_server.EnhancedGitHubIntegration"
            ...
            mock_integration_instance.handle_issue_event.return_value = {
                "handled": True,
                "updated_issues": [],
            }
            mock_integration_instance.handle_issue_comment_event.return_value = {
                "handled": True,
                "updated_issues": [],
            }
    ...
    @pytest.mark.asyncio
    async def test_webhook_route_missing_signature(
        self, webhook_server, aiohttp_client
    ):
        """Test webhook endpoint without signature header."""
        client = await aiohttp_client(webhook_server.app)

        resp = await client.post(
            "/webhook/github", json={"test": "data"}, headers={"X-GitHub-Event": "push"}
        )

        assert resp.status == 401

    @pytest.mark.asyncio
    async def test_webhook_route_invalid_signature(
        self, webhook_server, aiohttp_client
    ):
    ...
    async def test_webhook_route_valid_signature(self, webhook_server, aiohttp_client):
        ...
        signature = hmac.new(
            webhook_server.secret.encode(), payload_bytes, hashlib.sha256
        ).hexdigest()

        with patch.object(
            webhook_server, "_handle_event", new_callable=AsyncMock
        ) as mock_handle:
    ...
    def test_verify_signature_valid(self, webhook_server):
        ...
        signature = hmac.new(
            webhook_server.secret.encode(), payload, hashlib.sha256
        ).hexdigest()
    ...
    async def test_handle_event_push(self, webhook_server):
        ...
        event_data = {"ref": "refs/heads/main", "commits": [{"message": "Test commit"}]}

        with patch.object(
            webhook_server.github_integration, "handle_push_event"
        ) as mock_handle:
    ...
    async def test_handle_event_pull_request(self, webhook_server):
        ...
        with patch.object(
            webhook_server.github_integration, "handle_pull_request_event"
        ) as mock_handle:
    ...
    async def test_handle_event_issues(self, webhook_server):
        ...
        event_data = {
            "action": "opened",
            "issue": {"number": 1, "title": "Test Issue", "body": "Test content"},
        }

        with patch.object(
            webhook_server, "_handle_issues_event", new_callable=AsyncMock
        ) as mock_handle:
    ...
    async def test_handle_event_issue_comment(self, webhook_server):
        ...
        with patch.object(
            webhook_server, "_handle_issue_comment_event", new_callable=AsyncMock
        ) as mock_handle:
    ...
    async def test_handle_issues_event_closed(self, webhook_server):
        ...
        event_data = {"action": "closed", "issue": {"number": 1, "title": "Test Issue"}}
...
class TestWebhookCLI:
    ...
    def test_start_server(self, temp_dir):
        ...
        with patch("roadmap.webhook_server.RoadmapCore") as mock_core_class:
            with patch(
                "roadmap.webhook_server.GitHubWebhookServer"
                ...
                WebhookCLI.start_server(
                    host="localhost", port=8080, secret="test_secret"
                )
...
class TestWebhookServerIntegration:
    ...
    async def test_full_webhook_lifecycle(
        self, temp_workspace_with_core, aiohttp_client
    ):
        ...
        with patch(
            ...
            server = GitHubWebhookServer(roadmap_core=core, secret="test_secret")
            # Ensure the github_integration attribute is properly mocked
            server.github_integration = mock_integration_instance
    ...
    async def test_webhook_event_processing(
        ...
        with patch("roadmap.webhook_server.EnhancedGitHubIntegration"):
            server = GitHubWebhookServer(roadmap_core=core, secret="webhook_secret")
            ...
            signature = hmac.new(
                server.secret.encode(), payload_bytes, hashlib.sha256
            ).hexdigest()

            client = await aiohttp_client(server.app)

            with patch.object(
                server, "_handle_event", new_callable=AsyncMock
            ) as mock_handle:
                mock_handle.return_value = {"handled": True, "updated_issues": []}
